...
# Try to import setuptools_scm for automatic version management from Git tags
try:
...
def get_package_version():
    """
    Get package version using setuptools-scm if available, otherwise fall back
    to reading from __init__.py or git.

    This function supports:
    - Regular releases (e.g., 2.1.0)
    - Post-releases (e.g., 2.1.0.post1)
    - Development versions (e.g., 2.1.0.dev1+g1234567)
    """
    # Try setuptools-scm first (preferred method for accurate versioning)
    if HAS_SETUPTOOLS_SCM:
        try:
            version = scm_get_version(root=current_dir, relative_to=__file__)
            # For non-local builds, optionally strip local version identifier
            if not DG_USE_LOCAL_VERSION and '+' in version:
                version = version.split('+')[0]
            return version
        except Exception as e:
            print(f"Warning: setuptools-scm failed: {e}, falling back to legacy method")

    # Fallback: try to read from _version.py (generated by setuptools-scm during install)
    version_file = Path(current_dir) / 'deep_gemm' / '_version.py'
    ...
    # Legacy fallback: read version from __init__.py
    ...
    if DG_USE_LOCAL_VERSION:
        ...
        try:
            ...
            status_output = subprocess.check_output(status_cmd, cwd=current_dir).decode('ascii').strip()
            if status_output:
            ...
            revision = '+' + subprocess.check_output(cmd, cwd=current_dir).decode('ascii').rstrip()
