...
def setup_environment():
    """Optional fixture to set seeds, enable TF32, and print library info."""
    print('Library path:')
...
def test_gemm():
    ...
    for m in (64, 128, 4096):
        for (k, n) in [(7168, 2112), (1536, 24576), (512, 32768),
                       (16384, 7168), (7168, 4096), (2048, 7168)]:
            ...
            def test_func():
                x_fp8, y_fp8, out, _ = construct(m, k, n)
                deep_gemm.gemm_fp8_fp8_bf16_nt(x_fp8, y_fp8, out)
...
def test_m_grouped_gemm_contiguous():
    ...
    for num_groups, m, k, n in (
        ...
        diff = calc_diff(out, ref_out)
        assert diff < 0.001, f"{num_groups=}, m={m}, k={k}, n={n}, {diff:.5f}"

        # Performance
        def test_func():
            x_fp8, y_fp8, out, _ = construct_grouped(num_groups, m, k, n, is_masked=False)
            m_idx = torch.arange(0, num_groups, device='cuda', dtype=torch.int)
            m_idx = m_idx.unsqueeze(-1).expand(num_groups, m).contiguous().view(-1)
            deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_contiguous(x_fp8, y_fp8, out, m_idx)

        t = bench_kineto(test_func, 'fp8_gemm', suppress_kineto_output=True)
        print(f' > Performance (num_groups={num_groups}, m_per_group={m:4}, n={n:4}, k={k:4}): '
              f'{t * 1e6:4.0f} us | '
...
def test_m_grouped_gemm_masked():
    ...
    for (num_groups, m) in ((1, 1024), (2, 512), (4, 256)):
        for (k, n) in ((7168, 4096), (2048, 7168)):
            # Test correctness with random masks
            ...
            for _ in range(10):
                ...
                for j in range(num_groups):
                    ...
                    assert diff < 0.001, (
                        f"{m=}, {k=}, {n=}, group={j}, masked_m={masked_m[j]}, "
                        f"{num_groups=}, {diff:.5f}"
                    )

            # Performance with fully unmasked
            def test_func():
                x_fp8, y_fp8, out, _ = construct_grouped(num_groups, m, k, n, is_masked=True)
