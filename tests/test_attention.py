...
def test_mqa_logits():
    ...
    def enumerate_mqa_logits():
        ...
        if arch_major == 10:
            ...
            shapes = ((510, 130560), (512, 130560), (2048, 8192), (2048, 65536), (8192, 8192), (8192, 65536))
        elif arch_major == 12:
            fmts = ('mxfp4', 'fp8')
            shapes = ((128, 4096), (512, 8192), (2048, 8192), (4096, 8192))
        else:
            fmts = ('fp8', )
            shapes = ((2048, 8192), (2048, 65536), (8192, 8192), (8192, 65536))
                ...
                for weights_dtype in weights_dtypes:
                    ...
                    for compressed_logits, clean_logits in [(False, True), (True, False)]:
                        for seq_len, seq_len_kv in shapes:
                            ...
                            heads = (8, 12, 16, 20, 32, 64) if arch_major == 10 else ((16, 32, 64) if arch_major == 12 else (32, 64))
                            for num_heads in heads:
                                # The FP16-weights two-CTA kernel tiles Q in `128 / num_heads` rows
                                if weights_dtype == torch.float16 and 128 % num_heads != 0:
                                    continue
    ...
    for fmt, logits_dtype, weights_dtype, compressed_logits, clean_logits, seq_len, seq_len_kv, num_heads, head_dim, disable_cp in sample_mqa_cases('prefill', list(enumerate_mqa_logits())):
        ...
        kernel_weights = to_mqa_weights(weights * 0.1 if weights_dtype == torch.float16 else weights, weights_dtype)
        ...
        workspace = None
        # The FP16-weights kernel (nv_dev) has no scheduled-metadata path
        if get_arch_major() == 10 and weights_dtype != torch.float16:
...
def test_paged_mqa_logits():
                            ...
                            for use_2d_context_lens, clean_logits in [(True, False)]:
                                for batch_size in (256, 4096):
                                    ...
                                    for next_n in next_ns:
                                        for max_tokens_per_batch in ((1, 4, 6, 10) if is_varlen else (1, )):
                                            heads = (8, 12, 16, 20, 32, 64) if arch_major == 10 else ((16, 32, 64) if arch_major == 12 else (32, 64))
                                            if is_mxfp4:
    ...
    for is_varlen, fmt, logits_dtype, weights_dtype, block_kv, use_2d_context_lens, clean_logits, batch_size, next_n, max_tokens_per_batch, num_heads, head_dim, avg_kv in sample_mqa_cases('paged', list(enumerate_paged_mqa_logits())):
        ...
        metadata_kwargs = dict(
            ...
            num_sms=num_clusters, indices=indices,
        )
        kernel_kwargs = dict(
