...
def enumerate_k_grouped_contiguous(dtype: torch.dtype):
    arch = get_arch_major()
    if dtype == torch.bfloat16:
        sf_layout_list = [(128, 128)] if arch in (9, 12) else [(128, 128), (256, 256), (384, 384)]
    elif dtype == torch.float4_e2m1fn_x2:
        sf_layout_list = [(32, 256), (32, 512), (32, 768)]
    elif arch == 9:
        sf_layout_list = [(128, 128)]
    elif arch == 12:
        sf_layout_list = [(32, 128), (128, 128)]
    else:
        sf_layout_list = [(32, 128), (128, 128), (32, 256), (128, 256), (32, 384), (128, 384)]

    # SM90 FP8 and FP4 are K-major, SM120 FP8 supports both NT and TN, all other cases are MN-major.
    if (arch == 9 and dtype == torch.float8_e4m3fn) or dtype == torch.float4_e2m1fn_x2:
    ...
    # SM90 and SM120 kernels only support FP32 accumulation
    if arch in (9, 12):
