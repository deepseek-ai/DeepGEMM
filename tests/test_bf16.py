...
def test_gemm() -> None:
    ...
    for kernel_type, _, m, n, k, major_a, major_b, accumulate, out_dtype in enumerate_normal(torch.bfloat16):
        ...
        # shapes, a dot_kernel + reduce_1Block_kernel GEMV path for M=1 (SM120), plus a
        # split-K reduce kernel. Sum all matches for the total cuBLAS time.
        deep_gemm.use_deterministic_algorithms(False)
        cublas_t = sum(bench_kineto(lambda: deep_gemm.bf16_gemm_nt(a, b, d, c=c),
                                    ('nvjet', 'gemm', 'gemv', 'dot', 'reduce'),
