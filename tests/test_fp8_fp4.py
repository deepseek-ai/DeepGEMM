...
def test_gemm() -> None:
    ...
    for kernel_type, quant_config, m, n, k, major_a, major_b, accumulate, out_dtype, scores in \
        ...
        # On SM120 (consumer Blackwell) cuBLAS picks either an nvjet GEMM kernel or falls back
        # to a legacy sm89_xmma FP8 kernel depending on shape; match both, plus the split-K
        # reduce kernel. (SM90/SM100 use nvjet and never the sm89 fallback, so 'xmma' simply
        # matches nothing there.)
        cublas_times = bench_kineto(cublas_func, ('nvjet', 'bstensorop', 'xmma', 'reduce'),
...
def test_k_grouped_gemm_contiguous() -> None:
    ...
    # FP8: the K-major layout (SM90 always, SM120 optionally) uses the NT kernel, MN-major uses TN
    test_options = [(torch.float8_e4m3fn, QuantConfig(), None)]
    if arch_major == 10:
    ...
    for dtype, quant_config, fixed_gemm in test_options:
        ...
        for num_groups, m, n, major_a, major_b, real_ks_cpu, _, _, gran_k, k_alignment, use_psum_layout, accumulate, out_dtype in \
            ...
            gemm = fixed_gemm
            if gemm is None:
                gemm = deep_gemm.k_grouped_fp8_gemm_nt_contiguous if major_a == MajorTypeAB.KMajor \
