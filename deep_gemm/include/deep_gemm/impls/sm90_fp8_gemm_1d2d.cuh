...
sm90_fp8_gemm_1d2d_impl(float* sfb, int* grouped_layout,
    ...
    constexpr int MMA_N = kIsW4 ? (SHAPE_M != 0 ? SHAPE_M : BLOCK_M) : BLOCK_N;
    using WGMMA = typename FP8MMASelector<MMA_N>::type;
    ...
    static constexpr uint32_t SMEM_SFA_SIZE_PER_STAGE = (kIsW4 ? BLOCK_N : BLOCK_M) * sizeof(float);
    ...
    const uint32_t& smem_sfb_size = kIsW4 ? 0 : align<uint32_t>(shape_k_scales * (kMustUseUniformedScaleB ? 1 : 2) * sizeof(float), sizeof(Barrier));
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        if (warp_idx == kNumMathThreads / 32 + 2 and cute::elect_one_sync()) {
            ...
            while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
                ...
                for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                    ...
                    tma_copy<kIsW4 ? BLOCK_N : BLOCK_M, BLOCK_K, 0>(&tensor_map_sfa, &full_barrier,
                             smem_sfa[stage_idx], kIsW4 ? (n_block_idx * BLOCK_N) : (m_block_idx * BLOCK_M),
                             scheduler.template get_global_idx<kIsW4 or kWithGroupOffsetA, IndexType::SF_K>(shape_k_scales,
                                                                                  1,
                                                                                  k_block_idx,
                    ...
                    // Issue TMA B
                    tma_copy<BLOCK_K / WEIGHT_RATIO, BLOCK_N, kSwizzleBMode, __nv_fp8_e4m3, kIsBatchedMM>(&tensor_map_b, &full_barrier,
        ...
        auto a_desc = make_smem_desc(smem_a[0] + (kIsW4 ? 0 : math_wg_idx * WGMMA::M * BLOCK_K), 1);
        auto b_desc = make_smem_desc(smem_b[0] + (kIsW4 ? math_wg_idx * WGMMA::M * BLOCK_K : 0), 1);
        ...
        // for W4: Precompute thread-invariant values (hoisted out of k-loop)
        constexpr uint32_t NV = 16 / sizeof(__nv_fp8_e4m3);

        const uint32_t tidG = threadIdx.x % 128;
        const uint32_t tRow = (tidG & 15) | ((tidG >> 5) << 4);  // tidG % 16 + (tidG / 32) * 16
        const uint32_t tCol = ((tidG >> 4) & 1) * NV;            // ((tidG / 16) % 2) * NV
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            constexpr uint32_t WAVE_BLOCK_M = (kIsW4 ? BLOCK_N : BLOCK_M) <= WGMMA::M
                                              ? (kIsW4 ? BLOCK_N : BLOCK_M)
                                              : WGMMA::M * 2;
            DG_STATIC_ASSERT((kIsW4 ? BLOCK_N : BLOCK_M) % WAVE_BLOCK_M == 0, "Invalid block sizes");

            constexpr int WAVE_WGMMA = (kIsW4 ? BLOCK_N : BLOCK_M) / WAVE_BLOCK_M;
            ...
            DG_STATIC_ASSERT((kIsW4 ? BLOCK_N : BLOCK_M) >= 64 or kNumMathThreads == 128, "Only one math warp group for `BLOCK_M < 64`");
                ...
                dispatch_num_former_iters<0, kGap, kEnd>(kShouldOptimize ? num_former_iters : 0, [&](auto _) {
                    ...
                    for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                        ...
                        for (uint32_t local_idx = 0; local_idx < WAVE_WGMMA; ++ local_idx) {
                            ...
                            if constexpr (kIsW4) {
                                scale_b_0 = ld_shared(smem_sfa[stage_idx] + r_0 + m_offset);
                                scale_b_1 = ld_shared(smem_sfa[stage_idx] + r_1 + m_offset);
                            } else {
                                scale_a_0 = do_wgmma_store ? ld_shared(smem_sfa[stage_idx] + r_0 + m_offset) : 0;
                                scale_a_1 = do_wgmma_store ? ld_shared(smem_sfa[stage_idx] + r_1 + m_offset) : 0;
                            }
                            ...
                            if constexpr (kIsW4) {
                                using WGMMARS = typename FP8MMASelectorRS<MMA_N>::type;

                                uint32_t fragB[std::min(BLOCK_K / WGMMA::K, 2U)][4]; // 32 (4bit) data
                                ...
                                // make swizzle_col static, but not work?
                                const uint32_t pCol = permute_col<BLOCK_K / 2 * sizeof(__nv_fp8_e4m3), NV> (tRow, 0 * WGMMA::K / 2 + tCol);
                                ldmatrix<false, 4>(fragB[0], smem_b_w4_ptr + pCol);

                                fast_int4_to_fp8_convert (unpackB[0] + 0, fragB[0][0]);    // [0][0:4],   [8][0:4]
                                fast_int4_to_fp8_convert (unpackB[0] + 2, fragB[0][1]);    // [0][16:20], [8][16:20]
                                fast_int4_to_fp8_convert (unpackB[0] + 4, fragB[0][2]);    // [0][32:36], [8][32:36]
                                fast_int4_to_fp8_convert (unpackB[0] + 6, fragB[0][3]);    // [0][48:52], [8][48:52]

                                DG_STATIC_ASSERT((BLOCK_K / WGMMA::K) % 2 == 0, "Invalid unroll for w4 dequant.");

                                #pragma unroll
                                for (uint32_t k = 2; k < BLOCK_K / WGMMA::K; k += 2) { // stride: 2, for ldmatrix.{x4}
                                    const uint32_t pCol = permute_col<BLOCK_K / 2 * sizeof(__nv_fp8_e4m3), NV> (tRow, k * WGMMA::K / 2 + tCol);
                                    ldmatrix<false, 4>(fragB[(k / 2) % 2], smem_b_w4_ptr + pCol);

                                    fast_int4_to_fp8_convert (unpackB[(k / 2) % 2] + 0, fragB[(k / 2) % 2][0]);    // [0][0:4],   [8][0:4]
                                    fast_int4_to_fp8_convert (unpackB[(k / 2) % 2] + 2, fragB[(k / 2) % 2][1]);    // [0][16:20], [8][16:20]
                                    fast_int4_to_fp8_convert (unpackB[(k / 2) % 2] + 4, fragB[(k / 2) % 2][2]);    // [0][32:36], [8][32:36]
                                    fast_int4_to_fp8_convert (unpackB[(k / 2) % 2] + 6, fragB[(k / 2) % 2][3]);    // [0][48:52], [8][48:52]
                                ...
                                for (uint32_t k = 0; k < BLOCK_K / WGMMA::K; ++ k) {
                                    if constexpr (kIsW4) {
                                        ...
                                        b_desc.reg32_[0] = b_desc_base_lo + (m_offset * BLOCK_K + k * WGMMA::K) / 16;
                                        WGMMA::wgmma(b_desc, a_desc, accum, k);
                                    } else {
            ...
            for (uint32_t local_idx = 0; local_idx < WAVE_WGMMA; ++ local_idx) {
                ...
                for (auto i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                    ...
                    if constexpr (kSwizzleDMode > 0) {
                        ...
                        if constexpr (kIsW4) {
                            // In swap AB, each warp-group writes WGMMA::M N-elements per wave.
                            // i: 0 ~ (MMA_N / 8 - 1), row covers [0, BLOCK_M)
                            ...
                            // TMA atom index: each atom is (BLOCK_M, TMA_D_BLOCK_N) in smem_d.
                            // m_offset steps by WAVE_BLOCK_M (= WGMMA::M * 2 for multi-WG),
                            // math_wg_idx selects which WGMMA::M slice within the wave.
                            auto n_atom_idx = m_offset / WGMMA::M + math_wg_idx;
                            smem_ptr = reinterpret_cast<uint8_t*>(smem_d) +                     // Base pointer
                                    n_atom_idx * BLOCK_M * kSwizzleDMode +                      // Swizzle atom offset
                    ...
                    SM90_U32x2_STSM_N<nv_bfloat162>::copy<kIsW4>(
