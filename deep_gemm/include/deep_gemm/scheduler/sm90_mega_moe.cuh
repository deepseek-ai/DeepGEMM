...
// ============================================================================
// SM90 (Hopper) MegaMoE scheduler
// ----------------------------------------------------------------------------
// Ported from upstream PR https://github.com/sgl-project/DeepGEMM/pull/36
// (`sched::MegaMoEScheduler` / `sched::BlockPhase` on the old `dev` branch).
//
// This scheduler walks a *wave* of local experts at a time (grouped by
// `kNumExpertsPerWave`), alternating between all L1 (Linear1) blocks and all
// L2 (Linear2) blocks of the wave, over a persistent grid of `kNumSMs`
// single-CTA "tiles" (no 2-CTA cluster pairing, unlike SM100's
// `sched::MegaMoEScheduler`). It is intentionally simpler than SM100's
// ring-buffered, cluster-pair-interleaved scheduler in `mega_moe.cuh`, which
// is why it is kept as a separate class (`MegaMoESM90Scheduler`) and enum
// (`MegaMoESM90BlockPhase`) rather than reusing/extending the SM100 one: the
// two have incompatible state machines and workspace layouts
// (`layout::MegaMoESM90Workspace` vs. `layout::Workspace`).
// ============================================================================

namespace deep_gemm::sched {

// Computation phase for the current block (SM90 MegaMoE only; do not confuse with the
// unrelated SM100 `sched::BlockPhase`, which additionally has `SharedLinear1`/`SharedLinear2`).
enum class MegaMoESM90BlockPhase {
    None = 0,
    Linear1 = 1,
    Linear2 = 2
};
...
struct MegaMoESM90Scheduler {
    ...
    // Arrival counts
    const layout::MegaMoESM90Workspace& workspace;

    // Scheduler state
    MegaMoESM90BlockPhase next_phase = MegaMoESM90BlockPhase::Linear1;
    ...
    CUTLASS_DEVICE explicit MegaMoESM90Scheduler(const layout::MegaMoESM90Workspace& workspace): workspace(workspace) {
    ...
    CUTLASS_DEVICE cute::tuple<MegaMoESM90BlockPhase, uint32_t, uint32_t, uint32_t> get_next_block() {
        while (true) {
            if (current_local_expert_idx >= kNumExpertsPerRank)
                break;

            if (next_phase == MegaMoESM90BlockPhase::Linear1) {
                if (fetch_next_l1_block()) {
                    // Found a new L1 block
                    n_block_idx = block_idx - m_block_idx * kNumL1BlockNs;
                    // Jump to next block
                    block_idx += kNumSMs;
                    return {MegaMoESM90BlockPhase::Linear1, current_local_expert_idx, m_block_idx, n_block_idx};
                } else {
                    // L1 for the current wave is complete, transition to L2
                    next_phase = MegaMoESM90BlockPhase::Linear2;
                ...
                if (fetch_next_l2_block()) {
                    ...
                    // Jump to next block
                    block_idx += kNumSMs;
                    return {MegaMoESM90BlockPhase::Linear2, current_local_expert_idx, m_block_idx, n_block_idx};
                } else {
                    // Move to L1 of the next wave
                    next_phase = MegaMoESM90BlockPhase::Linear1;
                }
            }
        }

        // All waves and experts are fully processed
        return {MegaMoESM90BlockPhase::None, 0, 0, 0};
    }
    ...
    CUTLASS_DEVICE void for_each_block(Func&& func) {
        ...
        while (true) {
            ...
            if (block_phase == MegaMoESM90BlockPhase::None)
                break;

            func(block_phase, current_local_expert_idx,
                 block_phase == MegaMoESM90BlockPhase::Linear2 ? kNumL2BlockKs : kNumL1BlockKs,
