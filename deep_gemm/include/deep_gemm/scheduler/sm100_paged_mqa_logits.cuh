...
void sm100_paged_mqa_logits_metadata(const uint32_t num_requests,
    ...
    // An empty varlen batch may carry no requests at all. Handle it before
    // touching shared memory, and emit the same one-past-the-end sentinel
    // (q_token_idx = num_q_tokens_total = 0) for every scheduler boundary.
