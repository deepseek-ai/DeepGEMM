...
struct alignas(128) MegaMoESignals {
    ...
    // Legacy per-ring-block L2 full counters, used by the kernels that still track L1 -> L2
    // readiness with a count instead of `l2_full_mask` (NVFP4 MegaMoE)
    uint32_t l2_full_count[kNumMaxRingBlocks];
};
...
struct Workspace {
    ...
    // Legacy count-based L1 -> L2 readiness signal (NVFP4 MegaMoE)
    CUTLASS_DEVICE
    uint32_t* get_l2_full_count_ptr(const uint32_t& ring_block_idx = 0) const {
        return signals->l2_full_count + ring_block_idx;
    }
...
struct MegaMoEBuffer {
    ...
    MegaMoEBuffer(void* base,
        ...
        // NOTES: SF records are indexed MN-major by the kernels and never used as a TMA stride,
        // so they do not need the 16-byte record alignment (e.g. `intermediate_hidden = 2304`)
