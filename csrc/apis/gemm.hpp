...
static void fp8_fp4_gemm_nt(const std::pair<torch::Tensor, torch::Tensor>& a,
    ...
    if (arch_major == 12) {
        // SM120: AB-swap decision BEFORE SF transform (avoid double transform)
        auto a_data = a.first;
        auto b_data = b.first;
        ...
        // AB-swap: for M ∈ [1, 16], swap A↔B so small M becomes N (BN=16).
        // Uses cp.async for B tile when N_kernel < TMA minimum.
        ...
        // Resolve actual granularities from whichever recipe form is provided
        int ga, gb, gk;
        if (recipe.has_value()) {
