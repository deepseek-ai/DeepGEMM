...
// SM90 (Hopper) W4A8 MegaMoE entry point — Phase-0 scaffold.
//
// Accepts the same FP8 weights + per-128 float SF as `fp8_mega_moe` and
// dispatches to the W4A8 runtime. The runtime kernel is currently a verbatim
// copy of the FP8 kernel (only renamed); subsequent phases will:
//   1. swap weight dtype to packed MXFP4 (uint8) + per-32 E8M0 (uint8) SF
//   2. add `dequant_mxfp4_to_fp8` insertion in the math warp mainloop
//   3. apply per-32 E8M0 scale on WGMMA accumulator (Marlin-style)
//
// In Phase 0 this entry is bitwise equivalent to `fp8_mega_moe`. Used to
// verify build/link/run plumbing for the new kernel name before changing
// kernel logic.
static void w4a8_mega_moe(
    ...
    // Phase-0 input checks: identical to fp8_mega_moe.
    const auto num_tokens = static_cast<int>(y.size(0));
    const auto [rm, rn, rk] = recipe;
    DG_HOST_ASSERT(rm == 128 and rn == 128 and rk == 128);
    DG_HOST_ASSERT(activation == "swiglu");
    ...
    // Dispatch to W4A8 runtime (Phase-0: identical kernel to FP8).
