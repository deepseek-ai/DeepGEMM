...
static void sm90_bf16_gemm(const torch::Tensor& a,
    ...
    const SM90BF16GemmRuntime::Args& args = {
        ...
        .gemm_config = config,
        .launch_args = LaunchArgs({config.num_sms / config.k_slices, config.k_slices}, config.thread_config.num_threads,
                                  config.smem_config.smem_size,
                                  config.multicast_config.num_multicast, config.k_slices),
