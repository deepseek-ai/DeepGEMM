...
static void smxx_fp8_mqa_logits(const torch::Tensor& q,
    ...
    // SM90/SM100 allocate an extra mbarrier pair per math warp-group; SM120 does not.
...
class SM100FP8MQALogitsF16WeightsRuntime final
    : public LaunchRuntime<SM100FP8MQALogitsF16WeightsRuntime> {
public:
    struct Args {
        int seq_len;
        int seq_len_kv;
...
static void sm100_fp8_mqa_logits_f16_weights(
    ...
    //   smem_q[num_q_stages]      + smem_weights[num_q_stages]     (BLOCK_Q_2CTA
    //   weights) smem_kv[num_kv_stages]    + smem_kv_scales[num_kv_stages]
    //   (BLOCK_KV/2 per CTA) smem_kv_offsets[num_q_stages]  + barriers + tmem_ptr
    const int block_q_2cta = block_q * 2;
    const int smem_q_per_stage = block_q * num_heads * head_dim; // FP8 = 1 byte
    const int smem_weight_per_stage =
        block_q_2cta * num_heads * 2;                        // half = 2 bytes
    ...
    const int smem_kv_scale_per_stage =
        (smem_kv_scale_raw + 511) / 512 * 512;             // align to 512
    ...
    torch::Tensor cu_seq_len_k_start_and_end =
        torch::stack({cu_seq_len_k_start, cu_seq_len_k_end}, 1)
            .reshape({-1})
            .contiguous();

    const SM100FP8MQALogitsF16WeightsRuntime::Args args = {
        .seq_len = seq_len,
        .seq_len_kv = seq_len_kv,
        .max_seqlen_k = max_seqlen_k,
        .stride_logits = stride_logits,
    ...
    const auto code = SM100FP8MQALogitsF16WeightsRuntime::generate(args);
    const auto runtime =
        compiler->build("sm100_fp8_mqa_logits_f16_weights", code);
    SM100FP8MQALogitsF16WeightsRuntime::launch(runtime, args);
}
