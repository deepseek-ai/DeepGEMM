...
// `num_clusters` is the scheduler slot count: one entry per two-CTA cluster for
// next_n=4 multicast, one per SM otherwise (callers pre-divide).
...
static void __instantiate_kernel() {{
    ...
    // Launch: one CTA per SM, grouped into `num_kv_multicast`-CTA clusters
    jit->launch(
        kernel, {
            .num_smem_bytes = smem_size,
            .grid_dim = dim3(num_sms, 1, 1),
