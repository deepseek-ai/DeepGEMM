...
static torch::Tensor get_mn_major_tma_aligned_packed_ue8m0_tensor(const torch::Tensor& sf,
    ...
    if (batched_sf.is_contiguous()) {
        ...
        // Bound `block_mn` by the per-block shared memory capacity (e.g. SM120 has only 99 KB)
        const int max_smem = jit->device.get_num_smem_bytes() - psum_smem_elems * 4;
        const int block_mn = std::min(48, (max_smem / (smem_sf_k * 4) / 4) * 4);
