...
// ----------------------------------------------------------------------------
// Ported from upstream PR https://github.com/sgl-project/DeepGEMM/pull/36. This is the SM90
// counterpart of `SM100FP8FP4MegaMoERuntime`. The kernel itself lives in
// `deep_gemm/impls/sm90_fp8_mega_moe.cuh`.
//
// Differences from the SM100 path:
//   * Activations and weights are both FP8 (e4m3); no FP4.
//   * Activation/weight scale factors (SF) are float, not UE8M0 int + per-32 UTCCP layout. L1
//     activation SF and weight SF are per-128 K; the fused L1 epilogue writes L2 activation SF at
//     per-64 K granularity.
//   * No tensor memory: WGMMA accumulators are register-resident.
//   * No 2-CTA cluster: `cluster_size` is always 1.
//   * No shared-expert support (the PR this was ported from does not implement it for SM90).
// ============================================================================
...
public:
    struct Args {
        ...
        // Tensormaps for activations and weights. Weight scale factors use block (128, 128)
        // quantization and are loaded by the math warpgroup directly from global memory (no TMA
        // descriptor required).
...
static void sm90_fp8_mega_moe(
    ...
    // L1 output (post-SwiGLU FP8): N is halved. The correctness path stages this tile in plain
    // row-major SMEM before the TMA store. Later L2 TMA loads may still swizzle from this
    // row-major global buffer into their own SMEM tile.
    // The usual TMA store is issued per warpgroup, each writing a `WG_BLOCK_M` row tile from its
    // own SMEM offset. The m64n128 2-WG split-N decode path is different: both warpgroups stage
    // one joint 64-column L1-output tile and a single warpgroup issues the combined store, so the
    // descriptor must cover the full block_m x (block_n / 2) tile.
