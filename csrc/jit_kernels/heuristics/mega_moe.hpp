...
// Element sizes are in bits so that packed NVFP4 (4 bits) is representable
static int get_num_mma_elem_bits(const MmaKind& mma_kind) {
    switch (mma_kind) {
        ...
        default: DG_HOST_UNREACHABLE("Unsupported MegaMoE MMA kind");
    }
}

// Byte-addressable element size; NVFP4 is sub-byte and must go through the bit-based helpers
static int get_num_mma_elem_bytes(const MmaKind& mma_kind) {
    const int num_bits = get_num_mma_elem_bits(mma_kind);
    DG_HOST_ASSERT(num_bits % 8 == 0);
    return num_bits / 8;
}
...
static std::tuple<int, int, int, int, int> get_block_config_for_mega_moe(
    ...
    // `block_k` is 128 bytes worth of elements. NVFP4 currently reuses the FP8 x FP4
    // token thresholds without NVFP4-specific tuning; it keeps the byte footprint
    // while doubling K elements relative to FP8.
    const int block_k = 128 * 8 / get_num_mma_elem_bits(mma_kind);
...
static MegaMoEConfig get_mega_moe_config(
    ...
    // NOTES: FP8 activations, FP8 weights, and FP4 weights (unpacked to 8-bit in smem, or packed for NVFP4)
    ...
    // NOTES: NVFP4 keeps the 4 KiB pull granularity its kernel was tuned with
    const int num_max_pull_bytes = mma_kind == MmaKind::MXFP8FP4 ? 8192 : 4096;
